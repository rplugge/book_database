...
class Book
  ...
  def valid?
    ...
    if @name == nil || @name == ""
      ...
      false
    elsif location_check.include?(@location_id) == false
      false
    elsif genre_check.include?(@genre_id) == false
      false
    else
      true
    end
  ...
  def self.where_genre(genre)
    results = CONNECTION.execute("SELECT * FROM books WHERE genre_id = #{genre}")
